...
data = data[data['Date'] <= '2020-07-20']

N = 6e7
confirmed = data['Total_Current_Positive_Cases'].to_numpy() / N
recovered = data['Recovered'].to_numpy() / N
quarantined = data['Home_Confined'].to_numpy() / N
...
def SEIR_test():
    ...
    def plot():
        ...
        plt.subplot(2, 2, 1)
        plt.title('SEIR Model with beta = {:.5f}\nsigma = {:.5f}, gamma = {:.5f}'.format(fit_params[0], fit_params[1], fit_params[2]))

        plt.ylabel('proportion in Compartment')
...
def SEIRP_test():
    ...
    def plot():
        ...
        plt.title('SEIRP Model with beta = {:.5f}, sigma = {:.5f}\ngamma = {:.5f}, alpha = {:.5f}'.format(fit_params[0], fit_params[1], fit_params[2], fit_params[3]))
...
def SEIQR_test():
    ...
    ode_data = solve_ode_SEIQR(time2, ini + fit_params)


    def plot():
        plt.subplot(2, 2, 3)

        plt.title('SEIQR Model with beta = {:.5f}, sigma = {:.5f}\ngamma = {:.5f}, delta = {:.5f}'.format(fit_params[0], fit_params[1], fit_params[2], fit_params[3]))
...
def SEIQDRP_test():
    ...
    def plot():
        ...
        plt.title('SEIQDRP Model with alpha = {:.5f}, beta = {:.5f}\nsigma = {:.5f}, gamma = {:.5f}'.format(fit_params[0], fit_params[1], fit_params[2], fit_params[3]))
        ...
        plt.plot(time2, ode_data[:, 3], label='Quarantined', color='red')
        plt.plot(time2, ode_data[:, 1], label='Exposed', color='green')
        plt.scatter(time, dead, s=.4, label='Dead_real', color='black')
